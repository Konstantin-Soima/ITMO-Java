package ru.itmo.lesson.lesson15.task;

import ru.itmo.lesson.lesson15.User;
...
public class MapTask {
    ...
    private static List<String> getLogins(HashMap<String, String> firstTaskMap , String city){
    ...
    private static Map<String, Integer> getWords(List<String> words){
        HashMap<String, Integer> doubles = new HashMap<>();
        for (String word:
             words) {
            if (doubles.containsKey(word)){
                doubles.replace(word,doubles.get(word)+1);
            } else
            {
                doubles.put(word,1);
            }
